...
WATERFOX_SEARCH_DUMP_ROOT = Path("waterfox/browser/components/search/dumps")
WATERFOX_SEARCH_COLLECTIONS = {
    "search-config-icons",
...
def resolve_targets(root, required):
    ...
    for bucket, collection in required:
        ...
        targets.append(DumpTarget(bucket, collection, path, download))
    return targets
...
def load_local_dump(path):
    ...
    if not isinstance(data.get("data"), list):
        raise ValueError(f"{path} is missing a data list")
    if not isinstance(data.get("timestamp"), int):
        raise ValueError(f"{path} is missing an integer timestamp")
    return data


def validate_waterfox_search_attachments(target, data):
    if target.collection != "search-config-icons":
        return

    attachment_dir = target.path.parent / target.collection
    for record in data["data"]:
        if "attachment" not in record:
            continue
        record_id = record.get("id")
        if not isinstance(record_id, str):
            raise ValueError(f"{target.key} has an attachment record without an id")

        attachment = attachment_dir / record_id
        metadata = attachment_dir / f"{record_id}.meta.json"
        if not attachment.exists() or not metadata.exists():
            raise FileNotFoundError(
                f"{target.key} is missing bundled attachment files for {record_id}"
            )
...
def update_target(root, server, target, check, timeout):
    ...
    if not target.download:
        data = load_local_dump(target.path)
        validate_waterfox_search_attachments(target, data)
        print(f"validated Waterfox-owned {target.key} at {relpath}")
        return False
