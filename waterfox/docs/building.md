...
The script reads `WaterfoxSettingsPolicy.requiredOfflineDumps`, updates the source dumps from Mozilla Remote Settings, and validates the Waterfox search dumps without replacing them with Mozilla search data.
...
Each fresh build job runs `python3 scripts/update-remote-settings-dumps.py` before `./mach build`, so release artefacts use current offline Remote Settings records from Mozilla while leaving the Waterfox search dumps intact. The Windows x64 PGO job does not run this step because it profiles the instrumented artefact that was already built.
