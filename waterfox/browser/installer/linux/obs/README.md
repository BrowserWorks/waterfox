...
The tarball must exist on the CDN before OBS fetches it. For every production release, including prereleases, `.github/workflows/production.yml` moves the signed Linux tarball to the release CDN path and then posts a signed GitHub compatible `push` payload with `ref: refs/tags/<version>` to the OBS workflow endpoint.
