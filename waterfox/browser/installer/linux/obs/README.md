...
1. Create the `waterfox` and `waterfox-aarch64` packages in `home:MrAlex94:waterfox` and give every repository both `x86_64` and `aarch64`.
...
5. Publish the project's signing key (`osc signkey home:MrAlex94:waterfox`) on the website and CDN so users can import it.
