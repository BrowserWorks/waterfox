...
export const PrivateTab = {
  ...
  _initMenus(win) {
    ...
    // The link item markup is shared with webext-panels.xhtml, which does
    // not load waterfox.ftl; the id resolves here through browser.xhtml.
    ...
    linkItem.addEventListener("command", event => {
      win.gContextMenu?.openLinkInTab(event, this.container.userContextId);
    });

    // The places menu markup is shared with the Library window, where these
    // items stay hidden and untranslated; the ids resolve here through
    // waterfox.ftl in browser.xhtml.
