...
function createTreeTabsController(window) {
  ...
  const TreeTabsDnD = {
    ...
    prepareDrop(tabContainer, event, { draggedTab, movingTabs, dropEffect }) {
      ...
      // behind instead of travelling with it.
      if (event.ctrlKey) {
  ...
  const controller = {
    ...
    _restoreOpenedTabFromSession(tab) {
      if (tab.closing || !this._isEnabled()) {
        return;
      }
      // Startup/session restore tabs are handled by manual restore flow.
      if (lazy.TreeTabsStore._restoringWindows?.has(window)) {
        return;
      }
      // Only process tabs with persisted session tree metadata.
      const rawAncestors = lazy.TreeTabsMigration.readTabKey(tab, "ancestors");
      if (!rawAncestors) {
        return;
      }

      const restored = lazy.TreeTabsStore.tryRestoreTabFromSessionData(tab);
      if (!restored) {
        // Root tabs should not stay attached if auto-attach picked a parent.
        const tabData = lazy.TreeTabsStore.loadTabState(tab);
        if (
          tabData &&
          !tabData.ancestors?.length &&
    ...
    _handleTabOpen(event) {
      if (this._isEnabled() && event.target) {
        const tab = event.target;
        window.setTimeout(() => this._restoreOpenedTabFromSession(tab), 150);
      }
    ...
    _getTwistyTabFromEvent(event) {
      ...
      const twistyWidth = 16;
      const style = window.getComputedStyle(tab);
      const inlinePadding = this._getTwistyInlinePadding(tab, style);
      const direction = style.direction;
      if (direction == "rtl") {
        const hitStart = rect.right - inlinePadding - twistyWidth;
        ...
        const hitEnd = rect.left + inlinePadding + twistyWidth;
        if (event.clientX < hitStart || event.clientX > hitEnd) {
          return null;
        }
      }
