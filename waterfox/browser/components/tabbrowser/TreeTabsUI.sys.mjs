...
function createTreeTabsController(window) {
  ...
  const TreeTabsDnD = {
    ...
    _previewDropParent(event, draggedTab) {
      ...
      let parent = null;
      if (hovered && !draggedItem.multiselected && !draggedTab.multiselected) {
        parent = hovered;
      } else if (prev) {
        const prevLevel = service.getLevel(prev);
        const level = this._chooseLevel(
          service,
          prev,
          next,
          draggedItem,
    ...
    prepareDrop(tabContainer, event, { draggedTab, movingTabs, dropEffect }) {
      ...
      if (!sourceService || !logicalDraggedTab) {
        return state;
      }
    ...
    _syncSubtreeStripPosition(tab) {
      ...
      if (
        subtree.some(
          moving => moving.group || moving.pinned || moving.splitview
        ) ||
        anchor.group ||
        anchor.splitview
      ) {
    ...
    afterCrossWindowDrop(
      ...
      if (state?.crossWindowSnapshot) {
        ...
        const adoptedRoot = adoptedTabMap.get(state.logicalDraggedTab);
        if (adoptedRoot || adoptedDraggedTab) {
          return adoptedRoot || adoptedDraggedTab;
        }
      }
      return null;
    },
  ...
  const controller = {
    ...
    _scheduleAllGroupCleanup(delay = 100) {
      this._scheduleGroupCleanup(
        Array.from(window.gBrowser.tabs).filter(tab =>
          lazy.TreeTabsGroups.isGroupTab(tab)
        ),
        delay
      );
    },
    ...
    _scheduleGroupCleanup(tabs, delay = 100) {
      ...
      if (!this._groupCleanupTabs.size) {
        return;
      }
      if (this._groupCleanupTimer) {
        window.clearTimeout(this._groupCleanupTimer);
      ...
      this._groupCleanupTimer = window.setTimeout(() => {
        this._groupCleanupTimer = null;
        const pending = Array.from(this._groupCleanupTabs);
        this._groupCleanupTabs.clear();
        lazy.TreeTabsGroups.cleanupNeedlessGroupTabs(window, pending);
      }, delay);
    },
    ...
    _updateTab(tab, indentPx, maxVisualLevel) {
      ...
      const logicalTab = this._getLogicalTreeTab(tab);
      if (logicalTab != tab && logicalTab?._treeDescendantsTooltip) {
        tab._treeDescendantsTooltip = logicalTab._treeDescendantsTooltip;
        tab.dataset.treeDescendantsTooltip = "true";
      }
    },
    ...
    _scheduleDragHoverExpand(tab) {
      ...
      this._dragHoverExpandTimer = window.setTimeout(() => {
        ...
        if (tab.closing || !lazy.TreeTabsService.isCollapsed(tab)) {
          return;
        }
        this._withAutoCollapseSuppressed(() => {
          lazy.TreeTabsService.expandSubtree(tab);
