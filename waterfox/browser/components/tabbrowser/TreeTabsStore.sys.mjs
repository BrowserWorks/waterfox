...
function getTabIdentity(tab) {
  ...
  return tab?.linkedBrowser?.permanentKey?.id ?? null;
}
...
export const TreeTabsStore = {
  ...
  loadTabState(tab) {
    ...
    const state = {
      ancestors: this._readTabJSON(tab, "ancestors") || [],
  ...
  onWindowRestored(window) {
    ...
    this._cancelWindowSave(window);
    if (!this._windowHasTreeStructure(window)) {
      this._windowStates.delete(window);
      return;
    }
  ...
  tryRestoreTabFromSessionData(tab) {
    if (!this._isEnabled() || !tab) {
      return false;
    }
    ...
    // Restore parent attachment if this tab was a child.
    if (tabData.ancestors?.length) {
      const parentTab = this._findTabByReference(window, tabData.ancestors[0]);
      if (parentTab && parentTab !== tab && !parentTab.closing) {
        const options = {};
        const insertBefore = this._findTabByReference(
          window,
          tabData.insertBefore
        );
        if (insertBefore && insertBefore !== tab && !insertBefore.closing) {
          options.insertBefore = insertBefore;
        }

        if (!options.insertBefore) {
          const insertAfter = this._findTabByReference(
            window,
            tabData.insertAfter
          );
          if (insertAfter && insertAfter !== tab && !insertAfter.closing) {
            options.insertAfter = insertAfter;
          }
        }

        if (lazy.TreeTabsService.attachTab(tab, parentTab, options)) {
          didRestore = true;
        }
      }
    }
    ...
    // Reclaim saved children that are currently unattached/root tabs.
    if (tabData.children?.length) {
      for (const childRef of tabData.children) {
        const childTab = this._findTabByReference(window, childRef);
        if (
          childTab &&
          childTab !== tab &&
          !childTab.closing &&
          ...
          if (lazy.TreeTabsService.attachTab(childTab, tab)) {
            didRestore = true;
          }
        }
      }
  ...
  migrateFromExtension(window) {
    lazy.TreeTabsMigration.maybeMigrate();
    if (window) {
      this.loadWindowStructure(window);
    }
  },
  ...
  _windowHasTreeStructure(window) {
    for (const tab of this._getWindowTabs(window)) {
