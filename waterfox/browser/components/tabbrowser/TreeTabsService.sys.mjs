...
const PREF_AUTO_COLLAPSE_ON_ATTACH =
...
export const TreeTabsService = {
  ...
  init(window) {
    ...
    this._getWindowState(window, { create: true });
  },
  ...
  attachTab(child, parent, options = {}) {
    ...
    if (this._wouldExceedMaxDepth(state, parent)) {
      this.detachTab(child);
      return false;
    }
    ...
    if (getBoolPref(PREF_AUTO_COLLAPSE_ON_ATTACH, true)) {
  ...
  onTabClosed(tab) {
    ...
    const behavior = getIntPref(PREF_CLOSE_PARENT_BEHAVIOR, 1);
    if (behavior === 2) {
