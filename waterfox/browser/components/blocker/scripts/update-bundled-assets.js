...
const UBO_SCRIPTLET_OUTPUT_PATH = path.join(RESOURCES_DIR, "ubo-scriptlets.json");
...
/**
 * Build Waterfox scriptlet resources from uBO builtin scriptlets.
 *
 ...
 * 1. `import()` the uBO scriptlets module and read `builtinScriptlets`.
 ...
 *
 * @param {BuiltinScriptlet[]} builtinScriptlets
 * @returns {{
 *   aliases: string[],
 *   content: string,
 *   kind: { mime: string },
 ...
 * }[]}
 */
function buildScriptletResources(builtinScriptlets) {
  ...
  return builtinScriptlets
    ...
    .map(scriptlet => {
      ...
      for (const depName of requiredDependencies) {
        for (
          const recursiveDepName of dependencyMap[depName]?.dependencies ?? []
        ) {
      ...
      for (const depName of requiredDependencies.reverse()) {
        ...
        if (!depCode) {
          console.warn(`[update-bundled-assets] Missing dependency: ${depName}`);
          continue;
        }
...
async function updateDefaultFilters() {
  ...
  console.log(`Downloaded ${downloadCount} default-enabled filter list file(s).`);
}
...
async function updateScriptletResources() {
  ...
  if (!Array.isArray(builtinScriptlets)) {
    throw new Error("uBlock scriptlets module did not export builtinScriptlets");
  }
  ...
  await Bun.write(UBO_SCRIPTLET_OUTPUT_PATH, `${JSON.stringify(resources, null, 2)}\n`);

  const outputFile = Bun.file(UBO_SCRIPTLET_OUTPUT_PATH);
  if (!(await outputFile.exists()) || outputFile.size === 0) {
    throw new Error(`scriptlet output was not generated: ${UBO_SCRIPTLET_OUTPUT_PATH}`);
  }

  console.log(`Generated ${resources.length} scriptlet resources to ${UBO_SCRIPTLET_OUTPUT_PATH}`);
}
