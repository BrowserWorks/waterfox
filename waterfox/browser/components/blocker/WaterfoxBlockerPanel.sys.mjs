...
export const WaterfoxBlockerPanel = {
    ...
    this._forEachBrowserWindow(win => {
      this._forEachTab(win, tab => {
        ...
        try {
          ...
          // Use the local cache if the service is unavailable during startup.
          count = 0;
        }
  ...
  _readBlockedCount(browserId) {
    ...
    try {
      ...
      // Use the local cache if the service is unavailable during shutdown.
  ...
  _registerWidget() {
    ...
    try {
      ...
      // Missing placement is handled below by ensuring the default placement.
  ...
  _ensureDefaultPlacement() {
    ...
    try {
      ...
      // Placement is best-effort; users can still customise the toolbar.
      return false;
    }
  },
