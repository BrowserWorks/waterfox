...
function normalizeDomain(input) {
  ...
  try {
    const uri = Services.io.newURI(`http://${domain}`);
    domain = String(uri.asciiHost || uri.host || domain)
      .trim()
      .toLowerCase();
    if (domain.endsWith(".")) {
      domain = domain.slice(0, -1);
    ...
    // IP literals without brackets and internal hostnames may not parse as a
    // synthetic URI. Keep the normalized token and validate it below.
  }

  return domain;
}

function isAcceptableExceptionDomain(domain) {
  if (
    !domain ||
    domain.startsWith(".") ||
    domain.includes("/") ||
  ...
  try {
    const publicSuffix = Services.eTLD.getKnownPublicSuffixFromHost(domain);
    if (publicSuffix && publicSuffix === domain) {
      return false;
    }
  } catch (_) {
    // IP literals, localhost hosts, and private suffixes do not have a
    // known public suffix. These are acceptable explicit user exceptions.
  }
...
var gWaterfoxBlockerExceptionsManager = {
  ...
  _loadExceptions() {
    ...
    for (const exception of exceptions) {
      ...
      if (isAcceptableExceptionDomain(normalized)) {
  ...
  addException() {
    ...
    try {
      ...
      domain = normalizeDomain(uri.asciiHost || uri.host);
      if (!isAcceptableExceptionDomain(domain)) {
        throw new Error("Invalid host");
      }
    } catch (_) {
      // Invalid user-entered hosts use the standard invalid URI prompt.
      document.l10n
        .formatValues([
