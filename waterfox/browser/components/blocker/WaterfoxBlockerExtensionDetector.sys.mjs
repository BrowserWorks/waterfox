 ...
 * Module rationale:
 *
 * This module detects installed adblock extensions that conflict with
 * the blocker built into Waterfox and coordinates prompt handling for that state.
 *
 * Notification shown after an update uses the Spotlight upgrade message
 * pipeline from `AboutWelcomeParent.sys.mjs` and
 * `OnboardingMessageProvider.sys.mjs`. We clone a base upgrade message and
 * replace screen content with copy and actions for the blocker.
 *
 * Install interception uses `AddonManager.addInstallListener`, which is the
 * same decision hook that `abuseReport.sys.mjs` uses when an install starts.
 * `onInstallStarted` returns synchronously with a decision to allow or cancel, so
 * strings needed on synchronous paths are preloaded asynchronously at init and
 * then read from a small cache.
 */
...
/**
 * Detects conflicts between the built-in blocker and installed adblock
 * extensions.
 *
 * Runs conflict checks at startup and when add-on state changes, shows
 * Spotlight messaging after updates, and intercepts installs of conflicting
 * add-ons. Owns the localisation cache that synchronous install warnings
 * read from.
 */
export const WaterfoxBlockerExtensionDetector = {
  ...
  /**
   * Starts extension conflict detection and registers runtime observers.
   *
   * On the first call it wires AddonManager listeners, pref observers,
   * preloads synchronous prompt strings, and kicks off an initial detection
   * refresh pass. Safe to call more than once.
   */
  init() {
  ...
  /**
   * Stops extension conflict detection and unregisters all listeners.
   *
   * Safe to call more than once. Best-effort cleanup that tolerates shutdown
   * ordering.
   */
  uninit() {
    if (!this._initialized) {
  ...
  async _refreshDetectionNotifications() {
    ...
    // Only act when the builtin blocker is actually enabled. During the
    ...
    // Builtin blocker must not run alongside an extension adblocker.
  ...
  _removeStartupObserver() {
    ...
    try {
      ...
      // Startup observer may not have been registered.
    }
  ...
  /**
   * Builds and shows a Spotlight prompt after an update for blocker migration.
   *
   * Clones the base upgrade message, narrows it to a single screen, injects
   * localised title/subtitle/button labels, and opens a modal scoped to the
   * tab.
   *
   * @param {Window} win Browser window hosting the target tab.
   * @param {Element} browser Target browser element used as Spotlight host.
   * @param {string} extensionName Resolved conflicting extension display name.
   * @returns {Promise<boolean>} Spotlight return value, or `false` when the
   *          dialog is skipped or cannot be shown.
   */
  ...
  /**
   * Intercepts extension installs that may conflict with blocking built into Waterfox.
   *
   * AddonManager requires an immediate decision to allow or cancel here, so this
   * path uses preloaded strings and a synchronous prompt routine.
   *
   * @param {object} install Add-on install object from AddonManager.
  ...
  /**
   * Prompts the user to choose between builtin and extension blocking when
   * both are active after builtin blocking is re-enabled. Disables whichever
  ...
  async _handleBuiltInBlockerReenabled() {
    ...
    if (!allDisabled) {
      ...
      // User chose builtin — make sure the UI is visible so they can manage it.
  ...
  /**
   * Shows a synchronous install warning prompt for one conflicting add-on.
   *
   * This runs in a synchronous install path, so it reads preloaded strings
   * from the localisation cache.
   *
   * @param {Window} win Browser window used for prompt context in a modal scoped to the tab.
   * @param {object} addon Add-on metadata.
