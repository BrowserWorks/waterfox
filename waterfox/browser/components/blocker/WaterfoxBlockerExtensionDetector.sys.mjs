...
export const WaterfoxBlockerExtensionDetector = {
  ...
  _onInstallStarted(install) {
    ...
    // Don't prompt if this is a background update of an
  ...
  _onInstallEnded(install) {
    // Use a check so background updates of an already disabled
    // ad blocker extension do not force the built-in blocker off.
    const addon = install?.addon;
  ...
  /**
   * Prompts the user to choose between built-in and extension blocking when
   * both are active after built-in blocking is re-enabled. Disables whichever
