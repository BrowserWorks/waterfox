...
const HTML_SCRIPT_FILTER = htmlFilter([{ type: "css-selector", arg: "script" }]);
...
add_task(async function test_replace_response_filter_passes_large_body() {
  ...
  Assert.equal(
    body.length,
    LARGE_BODY.length,
    "Bodies over the replacement cap should pass through unchanged"
  );
  Assert.ok(
...
add_task(async function test_html_filter_skips_when_replace_expands_over_cap() {
  ...
  Assert.ok(
    text.length > EXPANDING_HTML.length,
