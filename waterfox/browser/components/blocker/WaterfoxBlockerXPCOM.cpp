...
/**
 * Initialises the blocker engine from full filter list text payloads.
 */
NS_IMETHODIMP
...
/**
 * Initialises the blocker engine from serialised cache bytes.
 */
NS_IMETHODIMP
...
/**
 * Checks a network request and returns normalised JSON with fields:
...
/**
 * Resolves adblock-rs CSP directives for document/subdocument contexts.
...
/**
 * Serialises the current engine state for cache persistence.
 */
NS_IMETHODIMP
...
/** Enables adblock-rs tags on the active engine. */
NS_IMETHODIMP
...
/** Disables adblock-rs tags on the active engine. */
NS_IMETHODIMP
...
/** Tests whether a tag exists in the current engine. */
NS_IMETHODIMP
...
/** Returns cosmetic resources JSON for a page URL. */
NS_IMETHODIMP
...
/** Resolves generic hide selectors from class/id snapshots and exceptions. */
NS_IMETHODIMP
...
/** Replaces the engine resource set with the provided JSON payload. */
NS_IMETHODIMP
...
/**
 * XPCOM constructor entry point. Follows the same pattern as
 * nsIUrlClassifierDBService: components.conf maps CID/contract here,
