 ...
 * including loads served from internal caches.
 *
 * Delegates decisions to the JS service bridge so bypass and exception
...
/**
 * XPCOM wrapper that exposes the native content classifier engine to JS.
 *
 * Implements `nsIWaterfoxBlockerEngine` and delegates to
 * `mozilla::ContentClassifierEngine` to bridge JS callers and the
 * Rust adblock engine.
 */
