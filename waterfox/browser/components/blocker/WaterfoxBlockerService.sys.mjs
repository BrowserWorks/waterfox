...
const SEARCH_PARTNER_DOMAINS = Object.freeze(["1.org", "search.waterfox.com"]);
...
function cssPropertyValue(element, property, pseudo = null) {
  ...
  try {
    ...
    // Detached parser documents do not always have a layout-backed view.
  }
  ...
  onStopRequest(request, statusCode) {
    if (!this.passThrough) {
      ...
      try {
        ...
        let rewrittenText = applyReplaceDirectives(
          originalText,
          this.replaceDirectives
        );
        const htmlFilters =
          originalBytes.length <= HTML_FILTER_MAX_BYTES &&
          rewrittenText.length <= HTML_FILTER_MAX_BYTES
            ? this.htmlFilters
            : [];
...
export const WaterfoxBlockerService = {
  ...
  /**
   * Resolves once an in-flight engine initialisation has settled. A no-op when
   * no initialisation is pending, so callers on the hot path only wait while
   * cache validation, list loading, or resource loading is still in progress.
   */
  ...
  getReplaceDirectives(
    ...
    try {
      ...
      return sanitizeStringList(rawDirectives, 100, 4096);
    } catch (err) {
