...
function parseListTtlMs(text) {
  ...
  // Only scan the first 20 lines — the header is always near the top.
...
export const WaterfoxBlockerService = {
        ...
        if (isExpired) {
          try {
            ...
            if (result.notModified) {
              ...
              // Keep the existing TTL — content hasn't changed.
              const prevTtl =
   ...
   *
   * @returns {Promise<Array<{url: string, filename: string, lastFetched: number, expiresAt: number, etag: string, lastModified: string}>>}
   */
