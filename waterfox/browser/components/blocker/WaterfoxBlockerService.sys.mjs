...
export const WaterfoxBlockerService = {
  ...
  _getSiteExceptions() {
    ...
    try {
      ...
      return parsed.map(toSafeDomain).filter(Boolean);
    } catch (_) {
      return [];
    }
  },
  ...
  async _initFromTextSourcesAndCache(descriptors, generation) {
    ...
    if (this._initGeneration !== generation || !this.isEnabled()) {
      return;
    }
  ...
  _startPeriodicListUpdates() {
    if (this._updateTimerId || !this.isEnabled()) {
      return;
    }
  ...
  async _updateListsIfNeeded(forceAll = false) {
    if (!this.isEnabled() || this._updateInProgress) {
      return;
    }
  ...
  async init() {
    ...
    Services.obs.addObserver(this, TOPIC_HTTP_ON_MODIFY_REQUEST);
    Services.obs.addObserver(this, TOPIC_HTTP_ON_EXAMINE_RESPONSE);
    Services.obs.addObserver(this, TOPIC_HTTP_ON_EXAMINE_CACHED_RESPONSE);
    Services.obs.addObserver(this, TOPIC_HTTP_ON_EXAMINE_MERGED_RESPONSE);
  ...
  observe(subject, topic, data) {
    ...
    switch (data) {
      ...
      default:
        // No eager engine rebuild needed.
        break;
    }
  },
  ...
  shouldBypassBlocking(loadingPrincipalDomain) {
    ...
    const partners = SEARCH_PARTNER_DOMAINS;

    return partners.some(p => {
      const normalizedPartner = toSafeDomain(p);
      return (
        normalizedPartner &&
        (domain === normalizedPartner ||
          domain.endsWith(`.${normalizedPartner}`))
      );
    });
  },
