...
export class WaterfoxBlockerChild extends JSWindowActorChild {
  ...
  async _onDOMWindowCreated() {
    ...
    // The blocker's enabled/bypass state is resolved by the parent when it
    // answers GetCosmeticResources (returning null), so a single round-trip is
    // enough — no separate IsEnabled query is needed before injection.
    let url;
    try {
