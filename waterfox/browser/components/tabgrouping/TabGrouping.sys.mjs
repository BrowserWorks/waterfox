...
export const TabGrouping = {
  ...
  observe(subject, topic, _data) {
    switch (topic) {
      ...
      case "sessionstore-windows-restored":
        // Resume grouping after session restore is complete
      ...
      case "browser-delayed-startup-finished":
        // Defensive: if sessionstore didn't notify (e.g., no restore), resume now
        if (this._suspended) {
          this._suspended = false;
        }
        try {
  ...
  _addCollapseListenerToWindow(window) {
    ...
    const collapseHandler = () => {
      if (this._suspended || !this._enabled) {
        return;
      }
